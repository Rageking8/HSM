namespace HSM.Core.State
{
        ...
        StateBase<TStateID>
    {
        ...
        private Dictionary<StateMessageMethod, ActionList>
            _allMethods = new();
        ...
        protected void AddAction(params ActionEntry[] entries)
        {
            ...
            foreach (ActionEntry entry in entries)
            {
                    ...
                    Enum.GetValues(typeof(StateMessageMethodFlag)))
                {
                    ...
                    bool result = Enum.TryParse<StateMessageMethod>(
                        flag.ToString(), out var method);
                    Assert(result, "`flag` is invalid");

                    // Check to see if the dictionary `allMethods`
                    // has the key `method`
                    bool keyExists = _allMethods.TryGetValue(method,
                        out var list);

                    // `method` key already exists in the dictionary
